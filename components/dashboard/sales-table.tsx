...
export function SalesTable({ sales, onEdit, onDelete, onViewDetails, onStatusChange, isLoading }: SalesTableProps) {
  const [searchTerm, setSearchTerm] = useState("")
  const [dateFilter, setDateFilter] = useState("")
  const [updatingStatus, setUpdatingStatus] = useState<string | null>(null)
  const [salespersonsMap, setSalespersonsMap] = useState<Record<string, Salesperson>>({})
  ...
  const loadSalespersons = async () => {
    ...
    const { data } = await supabase.from("salespersons").select("*").in("id", salespersonIds)
    ...
    <div className="space-y-4">
      ...
      {filteredSales.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          {sales.length === 0 ? "Nenhuma venda registrada ainda." : "Nenhum resultado encontrado."}
        </div>
      ) : (
                ...
                <TableRow key={sale.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => onViewDetails(sale)} title="Ver Detalhes">
                        <Eye className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => onEdit(sale)} title="Editar">
